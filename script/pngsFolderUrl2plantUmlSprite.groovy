...
final DEFAULT_SCALE = 0.5 //0.2
// final TMP_DIR = new File('/tmp/svgsFolderUrl2plantUmlSprites')
final TMP_DIR = new File('/tmp/pngsFolderUrl2plantUmlSprites')
TMP_DIR.mkdirs()
...
final SPRITES_LISTING = new SpritesListing(new File('sprites-list.md'), TMP_DIR)
...
def cli = new CliBuilder(usage: "${this.class.getSimpleName()}.groovy [options] <pngs URL>", stopAtNonOption: false, footer: "Usage example: ./${this.class.getSimpleName()}.groovy https://github.com/gilbarbara/logos/tree/master/logos")
...
static def listPngsUrls(baseUrl) {
...
static def downloadFile(url, workDir, useCache) {
    ...
    if (useCache && pngFile.exists()) {
        return pngFile
    }
    pngFile.delete()
    pngFile << new URL(url).text
    return pngFile
}
